      ...
      <div className={`hover:bg-neutral-800 rounded-full p-2 flex items-center transition hover:bg-neutral-900 transition-colors duration-700 ease-in-out gap-[20px]`}>
        <div className="relative">
          ...
          {notificationsCount > 0 && 
            <div className="absolute flex items-center justify-center top-0 right-0 translate-y-[-50%] p-[2px] bg-[#1d9bf0] rounded-full text-xs font-normal h-[14px] w-[14px]">{`${userNotificationCount > 50 ? '50+' : userNotificationCount}`}</div>
          }
        </div>
        <p className="hidden md:block font-normal text-xl">Notifications</p> 
      </div>
    </Link>
  )
}
...
const PopUpMessage = ({ timeout = 5000, text, setText, iconSrc }: IPopUpTimeout) => {
  ...
  return (
    <div className="overlay select-none fixed top-8 right-8 bg-[white] p-[16px] text-white flex items-center justify-center gap-2 rounded-full" onMouseEnter={() => setText(null)}>
      <img 
        className={`w-[15px] h-[15px]`} 
        src={iconSrc}
        alt="pop-up icon" 
      />
      <p className="ml-[3%] text-red-700 text-[11px]"> {text}</p>    
    </div>
  )
}

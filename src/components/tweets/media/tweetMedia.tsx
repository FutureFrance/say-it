    ...
    <div className="tweet_media mb-4 flex flex-wrap" onClick={e => e.stopPropagation()}>
      {
        ...
        ?
          <img 
            className={`w-[100%] max-h-[440px] rounded-[10px] cursor-pointer object-cover`} 
          ...
          <div className="flex gap-[1px]">
            <div className={`w-[50%] max-h-[440px]`}>
              {leftMedia.map((media, i) => (
                <img 
                  className={`object-cover w-[100%] rounded-[10px] cursor-pointer ${leftMedia.length > 1 ? i == 0 ? 'h-[50%] rounded-bl-none mb-[1px]' : 'h-[50%] rounded-tl-none' : 'h-[100%]'} ${leftMedia.length > 0 ? 'rounded-tr-none rounded-br-none' : ''}`} 
            ...
            <div className="w-[50%] max-h-[440px]">
              {rightMedia.map((media, i) => (
                <img
                  className={`object-cover w-[100%] rounded-[10px] cursor-pointer ${rightMedia.length > 1 ? i == 0 ? 'h-[50%] rounded-br-none mb-[1px]' : 'h-[50%] rounded-tr-none' : 'h-[100%] rounded-br-none'} ${leftMedia.length > 0 ? 'rounded-tl-none rounded-bl-none' : ''}`} 
                  onClick={() => handleImageClick(media.path)}
                  key={media.id} 
                  src={media.path} 
                />
